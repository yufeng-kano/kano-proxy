//! `custom_providers` rows (apps/api/src/db/custom_providers.ts). The slug namespace is
...
/// Combined cap across custom and CLI providers (apps/api/src/utils/custom_provider.ts).
