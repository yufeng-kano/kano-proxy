//! Validation and limits for user-defined model groups (apps/api/src/utils/model_group.ts,
...
/// `ordered` (apps/api/src/routing/strategy.ts `DEFAULT_STRATEGY`). Kept here until the
...
/// (apps/api/src/db/model_groups.ts `GroupTarget`).
