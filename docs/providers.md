...
- **Reasoning is exposed, not hidden.** Every `grok/*` request sets xAI's `include_reasoning: true` (verified 2026-08-02) — without it, xAI's Chat Completions surface returns no reasoning text at all, only `usage.completion_tokens_details.reasoning_tokens` as a count. With it: non-stream responses carry `message.reasoning_content`, streaming carries `delta.reasoning_content` deltas that arrive before any `content`/`tool_calls` deltas. OpenAI-surface clients see this automatically (near-passthrough adapter); the Anthropic surface converts it into a leading, **unsigned** `thinking` content block (no `signature` — this proxy has none to attach) — see [api.md](./api.md). Echoing a `thinking` block back in a later turn round-trips: its text folds back into `reasoning_content` on the corresponding OpenAI-shaped assistant message before replay upstream, which xAI accepts (verified 200). **Caveat:** xAI strips reasoning text server-side for requests from datacenter egress (Cloudflare included) — same request+token yields reasoning locally but none from a Worker — so in practice grok reasoning stays invisible until xAI relaxes that gate; see [api.md](./api.md) "xAI egress gate".
- **The tool-call loop guard (see [api.md](./api.md)) stays on as defense-in-depth** even with reasoning now exposed — a client isn't guaranteed to surface or act on `reasoning_content`/`thinking`, so the identical-tool-call symptom this proxy was built to catch can still happen with reasoning visible, not only when it was hidden.
- **Sampling:** `temperature` forwarded when the client sends it, else pinned to `1` explicitly — Anthropic and OpenAI both document `1` as their own default, while xAI's `/v1/chat/completions` documents none (a separate xAI surface, `/v1/responses`, was observed 2026-08-02 defaulting to `temperature 0.7` / `top_p 0.95` — exactly the kind of silent divergence pinning `1` avoids). `top_p` forwarded only when the client sends it, no default.
