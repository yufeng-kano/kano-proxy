...
- **Request body sent upstream (`/codex/responses`):**
  ...
  - `prompt_cache_key`: forwarded when the client sends a non-empty string (official OpenAI Chat Completions field), **fitted to OpenAI's 64-char limit** first — longer values are a hard `400 Invalid 'prompt_cache_key': string too long` upstream. Fit rule (deterministic, so a conversation keeps one stable value): trailing `_session_<uuid>` → that bare uuid; else ≤ 64 chars → verbatim; else SHA-256 hex of the key. Only the upstream body field is fitted — the replay session id and `session_id` header still use the full key. See `docs/api.md` "Codex Responses details". Pairs with deterministic account acquisition (`ORDER BY priority DESC, created_at DESC`, first non-benched account wins — see `db/accounts.ts` / `pool/acquire.ts`) so the same upstream ChatGPT account usually serves every turn, and a stable key produces real upstream prompt-cache hits. On `/anthropic` (no such wire field) the key is derived from Anthropic `metadata.user_id` — Claude Code's per-session id — under the same guard (string, trimmed, non-empty, ≤ 256 chars). The key also serves as the reasoning-replay session fallback below and feeds the `session_id` header: affinity headers first, then the key's trailing `_session_<uuid>` when it matches Claude Code's shape, then the raw key, then a per-request random UUID only when nothing else exists.
