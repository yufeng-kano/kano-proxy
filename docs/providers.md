...
Full cost analysis, platform comparison and open questions: [codex-relay-proposal.md](./codex-relay-proposal.md). This crosses the Cloudflare-only platform rule in `CLAUDE.md`, so it needs an explicit decision first — **it is proposed, not approved, and not implemented**. Four things to get right if it is built:
