...
> **xAI egress gate (verified 2026-08-02).** xAI honors `include_reasoning` selectively by client environment: an identical request+token that returns `reasoning_content` from a residential connection returns **zero** reasoning text when sent from Cloudflare's edge (controlled A/B: same token, headers, and body; only the egress differed — 200 either way, reasoning silently stripped, `reasoning_tokens` still reported). Since this proxy runs on Cloudflare Workers, grok reasoning text is currently **not observable in practice**; the request field, conversion mapping, and round-trip stay in place (zero-cost, and they light up automatically if xAI relaxes the gate), the token accounting above is unaffected, and the [loop guard](#degenerate-tool-call-loop-guard) is the operative defense for grok agent loops.
