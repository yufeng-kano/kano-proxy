...
Replace `<your-domain>` with the hostname you serve the proxy on (see [deployment.md](./deployment.md)). Admin UI and `GET /api/models` expose the live bases for the current host — nothing is hard-coded to a specific domain.
...
| `prompt_cache_key` | optional string, official OpenAI Chat Completions field. Forwarded to `codex`'s Responses `prompt_cache_key` when non-empty, **shortened to OpenAI's 64-char limit** first (see below); for `codex` it also feeds the `session_id` header, which is fitted to the **same** 64-char limit (the backend validates that header under the `prompt_cache_key` name), and the reasoning-replay session fallback, which keeps the full key because it never leaves the server. **Ignored** for `grok` and `claude-code`. The `/anthropic` wire format has no such field — the conversion derives it from Anthropic `metadata.user_id` instead (see "Codex Responses details") |
...
| image parts | Vision when upstream supports; Codex preserves the client's `image_url.detail` (`low` / `high` / `auto`) |
...
**Memory and flow control.** Native streaming Responses events pass through as bytes without buffering a whole line. Only proxy-generated OpenAI error lines are buffered for rewriting, up to 64 KiB; an oversized proxy error becomes a bounded `response.failed` error. Other provider/protocol converters assemble parsed SSE lines from fragments once, with a 16 MiB UTF-8 byte limit per line, and cancel the upstream on overflow rather than exhaust memory or silently drop data. This is an event parsing limit, not an HTTP request-size limit; native Responses passthrough has no corresponding line-size limit. Converter reads follow downstream demand, and cancellation stops upstream reads. Whole-response non-stream APIs still require memory proportional to the collected output. Proxy JSON ingress releases Hono's redundant cached raw text after parsing, retaining the parsed body for later consumers.
...
**Gemini function-call signatures on the Responses conversion path.** Gemini 3 requires each returned `functionCall.thoughtSignature` to be replayed unchanged on the matching function call in the next turn. Chat Completions has the proxy extension `tool_calls[].thought_signature`, but Responses has no portable field that the Codex CLI preserves for a custom provider. When a converted Chat tool call carries that extension, the proxy therefore emits an empty `reasoning` item immediately before the call, with `encrypted_content` set to a namespaced proxy marker containing that call's `call_id` and signature. The Codex CLI echoes that item on the next request; the request converter recognizes only this marker, removes it from ordinary reasoning handling, and restores the signature onto the matching Chat tool call. This is stateless transport metadata, not reasoning text or an OpenAI encrypted reasoning blob. Stream and non-stream output use the same marker, one per signed call, so parallel calls cannot exchange signatures; unrelated `reasoning.encrypted_content` remains ignored on the Chat conversion path.
...
- **`/anthropic` → `antigravity` (Gemini convert):** `thinking` block **with** `signature` whenever the upstream part carried a `thoughtSignature`; nothing is invented when it did not. The signature is echoed back as `thoughtSignature` on the next turn, which is what keeps multi-turn thinking valid — so unlike grok/codex there is no replay cache, the client can carry it itself. A replayed signature that is not Gemini-shaped (e.g. a UUID from a group's fallback target) is dropped, and a turn's first unsigned `tool_use` is replayed with Google's `skip_thought_signature_validator` placeholder — see [providers.md](./providers.md) § Antigravity. `thinking.type=disabled` suppresses the blocks entirely.
...
| `codex` | `200` with a **locally computed tiktoken count**: the server serializes the body's text (system, message blocks, tool names/descriptions/schemas) and runs the public OpenAI `o200k_base` tokenizer in process. No ChatGPT-backend call, no account acquired, no tokens consumed. This is a real tokenizer over an approximate serialization — the backend's own prompt framing adds a few percent the count cannot see. **Any failure degrades to `{"input_tokens": 0}`** — never an error status, because a failed count_tokens makes Claude Code fall back to probing with real `max_tokens: 1` generation requests in parallel (measured 2026-08-22; see the sentinel-zero row below). |
...
4. Resolve user’s pool for that provider (or custom slug). On a cloud edition that installed a pool extension, an **unpinned builtin** pool also includes the accounts other users share with the caller, merged into the caller's own priority order ([providers.md](./providers.md) § Routing module, [cloud-edition.md](./cloud-edition.md) § Pool extension); a standalone install never looks outside the caller's own rows.
...
Every streaming response, both surfaces, every provider, is relayed byte-for-byte from upstream — this proxy never buffers a whole completion before forwarding it — through a shared keepalive / eager-commit wrapper (`proxy/sse.ts`). Both wrappers are **demand-driven**: while the client is not consuming, the pump stops reading upstream (backpressure propagates instead of the remainder buffering in memory), and the keepalive/idle timers pause with it — a client-side stall is not an upstream silence gap, so it neither draws keepalives into an unread queue nor counts toward the 120s upstream idle timeout.
...
4. A client disconnect mid-wait cancels the stream (`onClose("cancel")`) so the row is logged as `client_abort` instead of vanishing with no log row (the pre-commit failure mode: the request torn down while still awaiting upstream headers).

**Non-stream** requests (`stream` absent/false) are unchanged: the proxy still waits for upstream headers and returns that HTTP status. The two surfaces deliver a non-stream upstream response differently, and deliberately so: `/anthropic` returns it **as received** — status, body, and every upstream header (rate-limit headers included), with only a clone peeked for usage — while `/openai/v1` Chat Completions rebuilds it with the upstream status, body, and `content-type` alone (audio transcriptions pass headers through). The one exception is an account borrowed through the pool extension ([cloud-edition.md](./cloud-edition.md) § Pool extension): rate-limit, `retry-after` and org/account-identity headers state the **owner's** quota, so they are dropped from every delivery path for those rows (the streamed passthrough's `anthropic-ratelimit-*` re-emission included). Own accounts are unaffected. When a non-stream request nevertheless gets an event-stream body back (an upstream that ignores `stream: false`), it is wrapped with keepalive/idle-timeout handling and passed through with the upstream status ("legacy attach", [logging.md](./logging.md)).

### Keepalive and idle timeout

- **Keepalive comments** (`: keepalive\n\n`, an SSE comment line clients ignore) fire every 10s of silence, so no intermediary mistakes a thinking model for a dead connection. Keepalives re-arm after **every** real upstream chunk, so any silence gap anywhere in the stream gets them (including the pre-upstream TTFB window under eager commit). The interval was 30s until 2026-08-19: clients with shorter idle timeouts (e.g. Claude Code) read a 30s silence gap as a dropped connection and retry, even though the upstream is still working.
...
| Codex relay platform rejects the request body before the relay app | 413 | **200** + terminal error frame | `request_too_large` |
| Request body over the proxy's 64 MiB ingress limit (see below) | 413 (plain text, before auth) | 413 (before auth) | — |
...
Session-auth JSON for the admin UI: the running server version, the newest published release, an update flag, and the sanitized release list — sourced from this repo's GitHub Releases, cached in process. `?refresh=true` bypasses the freshness window. Full contract (response shape, caching, stale-serve, sanitization) in [changelog.md](./changelog.md).
