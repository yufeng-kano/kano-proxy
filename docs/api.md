...
**Non-stream** requests (`stream` absent/false) are unchanged: the proxy still waits for upstream headers and returns that HTTP status. A non-stream upstream response is returned **as received** on both surfaces — status, body, and every upstream header (rate-limit headers included); the proxy only peeks at a clone of the body for usage. Until the dispatch consolidation (2026-09) the Chat Completions non-stream path rebuilt the response with only `content-type`, so the two surfaces differed here. When a non-stream request nevertheless gets an event-stream body back (an upstream that ignores `stream: false`), it is wrapped with keepalive/idle-timeout handling and passed through with the upstream status ("legacy attach", [logging.md](./logging.md)).
