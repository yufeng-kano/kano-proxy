...
| `prompt_cache_key` | optional string, official OpenAI Chat Completions field. Forwarded verbatim to `codex`'s Responses `prompt_cache_key` when non-empty (see below); for `codex` it also serves as the reasoning-replay session fallback and the `session_id` header fallback. **Ignored** for `grok` and `claude-code`. The `/anthropic` wire format has no such field — the conversion derives it from Anthropic `metadata.user_id` instead (see "Codex Responses details") |
...
| `/anthropic` → `codex` | Strip `cache_control` on convert. Client `metadata.user_id` (non-empty string ≤ 256 chars) → Responses `prompt_cache_key` + stable `session_id`, keeping a session's turns on one upstream cache shard (see "Codex Responses details"). A client-supplied id translated between wire formats — never invented server-side. |
