...
- TUI screen: slug (default: sanitized hostname, uniquified), API type `openai`/`anthropic`, target base URL (`openai`: include the `/v1`, e.g. `http://localhost:11434/v1`, the CLI appends `/chat/completions`; `anthropic`: the origin only, e.g. `http://localhost:11434`, the CLI appends `/v1/messages` — it appends only allowlisted suffixes and never guesses a prefix), optional local API key. It then probes the model list (`openai`: `GET <target>/models`; `anthropic`: `GET <target>/v1/models`, per `api.rs`): on success, a searchable model list opens whose **first row is "All models (follow local server)"** — the default, which stores **no filter**, so models added locally later appear automatically; selecting a subset stores an expose-whitelist instead. On probe failure it asks for model ids by hand (stored as the initial report; the agent's first connect overwrites it with truth) — a target that is not running yet must not block registration.
