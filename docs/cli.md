...
- Request body ≤ 32 MiB per request (`too_large` fault): `ws.send` exposes no drain signal, so a body arriving faster than the socket empties would queue unboundedly in memory — the hard cap is the honest request-side bound, the mirror of the response-side buffer below, until credit-based flow control (future work) covers both directions.
