...
- `routing/candidates.ts` merges `listShared` rows into unpinned builtin-pool candidates and into `GET /api/providers/:provider/accounts` in the same `(priority DESC, created_at DESC)` order (with the `share` descriptor; the core never includes the owner's upstream usage, but the route asks `listShared(…, { usage: true })` so the edition may attach its own `usage.windows` for the viewer, e.g. their remaining allowance, or the account's stored windows when its owner opted into showing them — routing never asks) and to the catalog's bound-provider set. Pinned group targets never resolve to a shared account.
...
- Unpause/delete/patch on a shared row from a non-owner return 403. The two ordering writes are the exception: `POST …/:id/promote` and `PUT …/accounts/order` delegate a shared row to `setSharedPriority`, which orders it inside the viewer's own pool only.
