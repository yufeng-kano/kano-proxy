...
Its `fixed` prop is the opt-in for a table with more columns than the card has width (Logs, and only Logs today): `table-layout: fixed`, so every column's `width` is a share of the card that no cell can argue with, and each cell clamped to the row's two lines. A page that turns it on gives **every** column a percentage summing to 100, and puts the full text of anything clampable on a `title`. Opt-in because it is a real constraint on the content — the five-column tables size themselves fine, and clamping their cells would cut text that fits today. The mobile card fallback opts back out of both: a card's value is the whole value.
...
- **One bounded card** filling the content region (same as Keys/Models), rows scrolling inside with a sticky header. The card scrolls **vertically only**: the table is fixed-layout and every column's track is a share of the card's width, so a long model id or account label can never widen the table. A horizontal scrollbar is not an acceptable fallback here — it parks Cost and Latency off-screen behind a scrollbar the operator has to find, and the row detail already carries anything the width cuts.
- **Row height and truncation**: a resting row is two lines of text tall, and each cell's content is clamped to those two lines. What does not fit is cut, never wrapped into a taller row and never allowed to widen the track. Every clamped cell carries its full text as a native `title` tooltip, so pointing at it reveals what was cut without reflowing the table; the row detail modal remains the keyboard-reachable full reading.
...
- **Status** is never color-only: success rows show the HTTP status quietly; failure rows show the `error_code` itself as **plain text** in the danger tone — never a badge. The code word *is* the identification, so a pill around it buys nothing and costs the width the code needs: in a track this narrow the pill clipped mid-word and left `upstream_unavailable` as an ellipsis inside a rounded border. The code wraps across the row's two lines (breaking inside the token, since a code carries no spaces) and keeps its full text on a `title`. `upstream_status` renders in the row detail, since `status_code` alone hides it for eager streams ([logging.md](./logging.md)).
...
- **The API key is named, never identified.** The response carries `api_key_name` and a boolean `api_key_removed` (the row points at a key deleted since the request ran); `api_key_id` is resolved server-side and **never leaves the Worker**, so no surface renders it. That id is not a credential — it is the `api_keys` primary key, while the `sk-kano-proxy-…` secret exists only as a SHA-256 hash and cannot be reconstructed from anything the API returns ([auth.md](./auth.md)) — but a long opaque string printed under the label "API key" reads as the key to whoever is looking at the screen, and nothing on this page needs it. `account_id` stays: a removed account leaves no other handle for tracing which pool member served the request.
...
- **Auto load more**: the next page fetches on approach — a sentinel after the last row, observed against the scrolling card, requests the next page roughly a viewport before the end so scrolling never stops at a button. One request in flight at a time, and the observer stops for good once `next_cursor` is `null`. The explicit **Load more** button stays: it is the keyboard path, the no-IntersectionObserver path, and what a failed auto-fetch falls back to (the fetch error stays beside it rather than retrying itself into a loop).
