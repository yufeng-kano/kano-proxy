...
- **Visible → fetch once, then restart the interval.** The one-shot catch-up is what keeps a returning user from reading a stale page; it still respects the localStorage TTL, and the 90s server cache absorbs the rest.
