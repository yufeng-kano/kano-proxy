...
- **Create/edit dialog:**
  ...
  - **Account select per target:** every target row carries an account control — default **Any account** (whole pool, provider's own priority), or one specific bound account of that target's provider, listed by display name (`custom_label` \|\| upstream label) from the same accounts data the Providers page loads (fetched lazily per provider, cached). Pinning is what lets two accounts of one provider be ordered as two targets; the picker must make "same model, different account" easy to add twice. Duplicate detection keys on `model`+account, matching the server. **Custom-endpoint targets offer only "Any account"** — the accounts API serves builtins only, and this MVP's custom providers hold a single key, so the pool *is* that key; the backend still accepts a pinned custom account id, the UI just has no picker for it yet (revisit when multi-key custom providers get UI).
