...
Route `/overview`; signed-in `/` redirects here. Data source: `GET /api/usage/summary?days=1|7|30` (session auth, see [auth.md](./auth.md)) aggregating `request_logs` — **live rows only, no fabricated numbers**; an empty range renders an explicit empty state with a link to Keys. The summary is **scoped to providers that still exist** (builtins + the user's live custom slugs); rows whose provider was deleted, and junk prefixes from invalid-model 400s, are excluded server-side.
