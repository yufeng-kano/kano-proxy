...
- Both sources are fetched server-side, trimmed to the four per-token rates this proxy uses, source-tagged before combination, and kept for **24h** in a small table so a restart does not refetch, with an in-process memo in front of it, so steady-state request handling touches neither. Only a source-tagged OpenRouter catalog entry can price an OpenRouter row; a LiteLLM `openrouter/<id>` entry, including one in a legacy combined snapshot, never can. An untagged legacy snapshot may stale-serve its non-OpenRouter prices, but the cron and admin usage summary recognize it as incomplete and force a source refresh rather than treating it as fresh.
- Source failures serve the last source-tagged copy regardless of age. A failed OpenRouter catalog fetch still refreshes LiteLLM-only prices, but leaves OpenRouter rows unpriced when no prior catalog copy exists; with no applicable copy or source entry, costs are `NULL` (unknown), never guessed. Pricing must never fail or delay a proxied request — proxy request logging never fetches; refreshes run from the daily cron or the admin usage summary, off the critical path.
...
2. For an `openrouter/...` row, exact match only on a source-tagged OpenRouter catalog entry for `openrouter/<full upstream id>`. This preserves the OpenRouter provider-specific rate and prevents a LiteLLM, bare/provider, or unrelated provider price from pricing an OpenRouter request.
...
6. Effort/thinking tier suffix fallback: for Antigravity or Gemini-family models, strip trailing `-high`, `-medium`, `-low`, `-thinking`, `-tiered`, `-thought` (and `-thinking-{high,medium,low}`) on the segment (`gemini-3.7-flash-high` → `gemini-3.7-flash`).
7. Preview suffix fallback: for Antigravity or Gemini-family models, try candidate with `-preview` appended (`gemini-3-flash` → `gemini-3-flash-preview`) or stripped.
...
- The window sum runs behind a 60s in-process memo per key, so a busy key does not add an aggregate query to every request.
- Fail-open on infrastructure error (a database hiccup must not take the proxy down), fail-closed on a real over-limit read — consistent with the rate-limit guardrail in `CLAUDE.md`.
