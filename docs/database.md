...
| api_key_name | TEXT | nullable; the key's `api_keys.name` **as it was when the row was written** (`0016_request_log_names.sql`). The Logs page prefers the live name and falls back to this when the key was deleted since, so a removed key still reads by its last name. `NULL` on rows written before the column existed or when the key was already gone at write time |
| account_label | TEXT | nullable; the serving account's display label (`custom_label` \|\| `label`) as it was at write time (`0016_request_log_names.sql`). Same read rule: live label first, this as the fallback for a removed account |
...
Name snapshots (`api_key_name`, `account_label`) added in `0016_request_log_names.sql`; the writer reads both by primary key just before the insert, off the request path, so a key or account deleted in that window simply leaves `NULL`. Cache columns added in `0003_request_log_cache_tokens.sql`; `cost` and the spend-limit index `request_logs_api_key_created_idx` on `(api_key_id, created_at)` added in `0004_api_key_spend_limits.sql`. Dashboard range queries are covered by the existing `request_logs_user_created_idx` on `(user_id, created_at)` from `0001_init.sql`.
