...
| GET | `/api/usage/summary` | Aggregated `request_logs` for the dashboard; `?from=` & `?to=` ISO strings with `?grain=hour|day`, or legacy `?days=` 1, 7, or 30 (default 7). `models[]` is per **(provider, model)** — no account/alias dimension (see [admin-ui.md](./admin-ui.md) § Series shape) |
