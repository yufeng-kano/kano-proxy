...
| PUT | `/api/custom-providers/:id` | Update — body `{name?, slug?, base_url?, api_key?, count_tokens_url?, models_mode?, manual_models?}`; `format` is immutable (`400` if a differing value is sent). A `slug` equal to the stored one is a no-op; a differing one **renames the model prefix**: trimmed and lowercased, validated like create (shape and reserved list → `400`; taken by another of the caller's custom or CLI providers → `409`), then the provider row, its `upstream_accounts` rows, its `provider_settings` row and every `<old>/<model>` group target are rewritten in one transaction ([providers.md](./providers.md) § Custom endpoints) before the other fields apply. Omitted or empty `api_key` keeps the stored key; a non-empty `api_key` re-encrypts and replaces it in place (same account row) |
