...
- **PKCE S256 required**: the verifier is generated by the proxy at start, only the challenge goes to `usercode`, and the verifier is sent at exchange with `redirect_uri=https://auth.openai.com/deviceauth/callback` (a fixed protocol constant here, not a page anyone visits).
- Start response carries `device_code`, `user_code`, and `verification_uri` / `verification_uri_complete` (`https://auth.openai.com/codex/device`); the poll returns `authorization_code` on approval, `authorization_pending` / `slow_down` until then (`slow_down` widens the interval).
- Same `client_id` as the CLI (`CODEX_OAUTH_CLIENT_ID` overrides) and the same scopes; the refresh path in `providers/codex.ts` is unchanged, since the tokens are ordinary OAuth tokens.
