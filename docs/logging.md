...
| `/openai/v1/responses` native path (all-codex candidates) | collected `response.completed` → `response.usage` (`input_tokens`, `output_tokens`, `input_tokens_details.cached_tokens`) | passthrough parse of the `response.completed` event while piping; the OpenAI sniffer recognizes that event alongside chat chunks, and it doubles as the completion signal. On a real Codex CLI turn that event is routinely longer than the sniffer's 256 KiB line cap (the ChatGPT backend echoes the request into it), so it is never buffered: the sniffer identifies it by its `event:` line or leading `"type"` and scans its small `usage` object out of the passing bytes — see the rules below |
...
- Capture never rewrites or buffers the client stream: streamed bodies are inspected chunk-by-chunk as they pass through the existing keepalive pipe, holding only a bounded partial-line carry (256 KiB). A line that outgrows the carry is **skipped, not fatal** — capture resumes on the next line. The one over-long line that matters is a Responses `response.completed` / `response.incomplete` event on the native codex path: recognized by its `event:` line or its leading `"type"`, its `usage` object (a few hundred bytes; scanned through a 4 KiB window, validated as an object carrying numeric token counts so a tool schema with a `usage` property cannot be mistaken for it) is picked out of the bytes as they pass, and the line still counts as the completion signal. Through v4.8.1 an over-long line abandoned capture outright, so every real Codex CLI turn on that path logged as `incomplete_stream` with NULL tokens while the client had its usage all along. A parse failure still degrades to `NULL` token fields, never a failed request.
